...
bool ChatHandler::HandleCooldownCommand(const char *args)
{
    ...
    else
    {
        ...
        {
            PSendSysMessage(LANG_UNKNOWN_SPELL, target == m_session->GetPlayer() ? GetDarkmoonCoreString(LANG_YOU) : tNameLink.c_str());
            SetSentErrorMessage(true);
            return false;
        }

        target->RemoveSpellCooldown(spell_id, true);
        PSendSysMessage(LANG_REMOVE_COOLDOWN, spell_id, target == m_session->GetPlayer() ? GetDarkmoonCoreString(LANG_YOU) : tNameLink.c_str());
   }
    return true;
}
...
bool ChatHandler::HandleAddItemSetCommand(const char *args)
{
    ...
    sLog->outDetail(GetDarkmoonCoreString(LANG_ADDITEMSET), itemsetId);
    ...
    for (uint32 id = 0; id < sSkillLineStore.GetNumRows(); id++)
    {
        ...
        if (skillInfo)
        {
            ...
            if (!name.empty())
            {
                ...
                {
                    knownStr = GetDarkmoonCoreString(LANG_KNOWN);
                    uint32 curValue = target->GetPureSkillValue(id);
                    uint32 maxValue  = target->GetPureMaxSkillValue(id);
                    uint32 permValue = target->GetSkillPermBonusValue(id);
                    uint32 tempValue = target->GetSkillTempBonusValue(id);

                    char const* valFormat = GetDarkmoonCoreString(LANG_SKILL_VALUES);
    ...
    for (uint32 id = 0; id < sSpellStore.GetNumRows(); id++)
    {
        ...
        if (spellInfo)
        {
            ...
            if (loc < TOTAL_LOCALES)
            {
                ...
                if (rank)
                    ss << GetDarkmoonCoreString(LANG_SPELL_RANK) << rank;
                ...
                if (talent)
                    ss << GetDarkmoonCoreString(LANG_TALENT);
                if (passive)
                    ss << GetDarkmoonCoreString(LANG_PASSIVE);
                if (learn)
                    ss << GetDarkmoonCoreString(LANG_LEARN);
                if (known)
                    ss << GetDarkmoonCoreString(LANG_KNOWN);
                if (active)
                    ss << GetDarkmoonCoreString(LANG_ACTIVE);
                        ...
                        if (target)
                        {
                            ...
                            if (status == QUEST_STATUS_COMPLETE)
                            {
                                if (target->GetQuestRewardStatus(qinfo->GetQuestId()))
                                    statusStr = GetDarkmoonCoreString(LANG_COMMAND_QUEST_REWARDED);
                                else
                                    statusStr = GetDarkmoonCoreString(LANG_COMMAND_QUEST_COMPLETE);
                            }
                            else if (status == QUEST_STATUS_INCOMPLETE)
                                statusStr = GetDarkmoonCoreString(LANG_COMMAND_QUEST_ACTIVE);
                        }
        ...
        if (factionEntry)
        {
            ...
            if (!name.empty())
            {
                ...
                if (repState)                               // and then target != NULL also
                {
                    ...
                    std::string rankName = GetDarkmoonCoreString(index);

                    ss << " " << rankName << "|h|r (" << target->GetReputationMgr().GetReputation(factionEntry) << ")";

                    if (repState->Flags & FACTION_FLAG_VISIBLE)
                        ss << GetDarkmoonCoreString(LANG_FACTION_VISIBLE);
                    if (repState->Flags & FACTION_FLAG_AT_WAR)
                        ss << GetDarkmoonCoreString(LANG_FACTION_ATWAR);
                    if (repState->Flags & FACTION_FLAG_PEACE_FORCED)
                        ss << GetDarkmoonCoreString(LANG_FACTION_PEACE_FORCED);
                    if (repState->Flags & FACTION_FLAG_HIDDEN)
                        ss << GetDarkmoonCoreString(LANG_FACTION_HIDDEN);
                    if (repState->Flags & FACTION_FLAG_INVISIBLE_FORCED)
                        ss << GetDarkmoonCoreString(LANG_FACTION_INVISIBLE_FORCED);
                    if (repState->Flags & FACTION_FLAG_INACTIVE)
                        ss << GetDarkmoonCoreString(LANG_FACTION_INACTIVE);
                }
                else
                    ss << GetDarkmoonCoreString(LANG_FACTION_NOREPUTATION);
        ...
        if (MapInfo)
        {
            ...
            if (loc < TOTAL_LOCALES)
            {
                ...
                if (MapInfo->IsContinent())
                    ss << GetDarkmoonCoreString(LANG_CONTINENT);

                switch(MapInfo->map_type)
                {
                    case MAP_INSTANCE:      ss << GetDarkmoonCoreString(LANG_INSTANCE);      break;
                    case MAP_BATTLEGROUND:  ss << GetDarkmoonCoreString(LANG_BATTLEGROUND);  break;
                    case MAP_ARENA:         ss << GetDarkmoonCoreString(LANG_ARENA);         break;
                }

                if (MapInfo->IsRaid())
                    ss << GetDarkmoonCoreString(LANG_RAID);

                if (MapInfo->SupportsHeroicMode())
                    ss << GetDarkmoonCoreString(LANG_HEROIC);
                ...
                if (MapInfo->IsMountAllowed())
                    ss << GetDarkmoonCoreString(LANG_MOUNTABLE);
...
bool ChatHandler::HandleNearGraveCommand(const char *args)
{
    ...
    if (graveyard)
    {
        ...
        std::string team_name = GetDarkmoonCoreString(LANG_COMMAND_GRAVEYARD_NOTEAM);

        if (g_team == 0)
            team_name = GetDarkmoonCoreString(LANG_COMMAND_GRAVEYARD_ANY);
        else if (g_team == HORDE)
            team_name = GetDarkmoonCoreString(LANG_COMMAND_GRAVEYARD_HORDE);
        else if (g_team == ALLIANCE)
            team_name = GetDarkmoonCoreString(LANG_COMMAND_GRAVEYARD_ALLIANCE);
...
bool ChatHandler::HandleListAurasCommand (const char * /*args*/)
{
    ...
    char const* talentStr = GetDarkmoonCoreString(LANG_TALENT);
    char const* passiveStr = GetDarkmoonCoreString(LANG_PASSIVE);
...
bool ChatHandler::HandleBanInfoCharacterCommand(const char *args)
{
    ...
    do
    {
        ...
        std::string bantime = permanent ? GetDarkmoonCoreString(LANG_BANINFO_INFINITE) : secsToTimeString(fields[1].GetUInt32(), true);
        PSendSysMessage(LANG_BANINFO_HISTORYENTRY,
            fields[0].GetCString(), bantime.c_str(), active ? GetDarkmoonCoreString(LANG_BANINFO_YES) : GetDarkmoonCoreString(LANG_BANINFO_NO), fields[4].GetCString(), fields[5].GetCString());
    }
    while (result->NextRow());
...
bool ChatHandler::HandleBanInfoHelper(uint32 accountid, char const* accountname)
{
    ...
    do
    {
        ...
        std::string bantime = permanent ? GetDarkmoonCoreString(LANG_BANINFO_INFINITE) : secsToTimeString(fields[1].GetUInt64(), true);
        PSendSysMessage(LANG_BANINFO_HISTORYENTRY,
            fields[0].GetCString(), bantime.c_str(), active ? GetDarkmoonCoreString(LANG_BANINFO_YES) : GetDarkmoonCoreString(LANG_BANINFO_NO), fields[4].GetCString(), fields[5].GetCString());
    } while (result->NextRow());
...
bool ChatHandler::HandleBanInfoIPCommand(const char *args)
{
    ...
    PSendSysMessage(LANG_BANINFO_IPENTRY,
        fields[0].GetCString(), fields[1].GetCString(), permanent ? GetDarkmoonCoreString(LANG_BANINFO_NEVER) : fields[2].GetCString(),
        permanent ? GetDarkmoonCoreString(LANG_BANINFO_INFINITE) : secsToTimeString(fields[3].GetUInt64(), true).c_str(), fields[4].GetCString(), fields[5].GetCString());
