...
bool ChatHandler::HandleAnnounceCommand(const char* args)
{
    ...
    char buff[2048];
    sprintf(buff, GetDarkmoonCoreString(LANG_SYSTEMMESSAGE), args);
...
bool ChatHandler::HandleNotifyCommand(const char* args)
{
    ...
    std::string str = GetDarkmoonCoreString(LANG_GLOBAL_NOTIFY);
    str += args;
...
bool ChatHandler::HandleGMNotifyCommand(const char* args)
{
    ...
    std::string str = GetDarkmoonCoreString(LANG_GM_NOTIFY);
    str += args;
...
bool ChatHandler::HandleGMVisibleCommand(const char* args)
{
    if (!*args)
    {
        PSendSysMessage(LANG_YOU_ARE, m_session->GetPlayer()->isGMVisible() ?  GetDarkmoonCoreString(LANG_VISIBLE) : GetDarkmoonCoreString(LANG_INVISIBLE));
        return true;
    }
...
bool ChatHandler::HandleSummonCommand(const char* args)
{
    ...
    else
    {
        ...
        PSendSysMessage(LANG_SUMMONING, nameLink.c_str(), GetDarkmoonCoreString(LANG_OFFLINE));
...
bool ChatHandler::HandleModifyEnergyCommand(const char* args)
{
    ...
    sLog->outDetail(GetDarkmoonCoreString(LANG_CURRENT_ENERGY), chr->GetMaxPower(POWER_ENERGY));
...
bool ChatHandler::HandleModifyMoneyCommand(const char* args)
{
    ...
    if (addmoney < 0)
    {
        ...
        sLog->outDetail(GetDarkmoonCoreString(LANG_CURRENT_MONEY), moneyuser, addmoney, newmoney);
        if (newmoney <= 0)
        {
    ...
    sLog->outDetail(GetDarkmoonCoreString(LANG_NEW_MONEY), moneyuser, addmoney, chr->GetMoney());
...
bool ChatHandler::HandleWhispersCommand(const char* args)
{
    if (!*args)
    {
        PSendSysMessage(LANG_COMMAND_WHISPERACCEPTING, m_session->GetPlayer()->isAcceptWhispers() ?  GetDarkmoonCoreString(LANG_ON) : GetDarkmoonCoreString(LANG_OFF));
        return true;
    }
...
/*    MapEntry const * me = sMapStore.LookupEntry(tele->mapId);
    ...
    else
    {
        ...
        PSendSysMessage(LANG_TELEPORTING_TO, nameLink.c_str(), GetDarkmoonCoreString(LANG_OFFLINE), tele->name.c_str());
