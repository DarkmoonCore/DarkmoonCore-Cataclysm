...
bool ChatHandler::HandleModifyRepCommand(const char * args)
{
    ...
    if ((amount == 0) && (rankTxt[0] != '-') && !isdigit(rankTxt[0]))
    {
        ...
        {
            std::string rank = GetDarkmoonCoreString(ReputationRankStrIndex[r]);
            if (rank.empty())
                continue;
...
bool ChatHandler::HandlePInfoCommand(const char* args)
{
    ...
    std::string username = GetDarkmoonCoreString(LANG_ERROR);
    std::string email = GetDarkmoonCoreString(LANG_ERROR);
    std::string last_ip = GetDarkmoonCoreString(LANG_ERROR);
    uint32 security = 0;
    std::string last_login = GetDarkmoonCoreString(LANG_ERROR);
    ...
    PSendSysMessage(LANG_PINFO_ACCOUNT, (target?"":GetDarkmoonCoreString(LANG_OFFLINE)), nameLink.c_str(), GUID_LOPART(target_guid), username.c_str(), accId, email.c_str(), security, last_ip.c_str(), last_login.c_str(), latency);
...
bool ChatHandler::HandleCharacterReputationCommand(const char* args)
{
    ...
    for (FactionStateList::const_iterator itr = targetFSL.begin(); itr != targetFSL.end(); ++itr)
    {
        ...
        std::string rankName = GetDarkmoonCoreString(ReputationRankStrIndex[rank]);
...
bool ChatHandler::HandleLookupEventCommand(const char* args)
{
    ...
    for (uint32 id = 0; id < events.size(); ++id)
    {
        ...
        if (Utf8FitTo(descr, wnamepart))
        {
            ...
            char const* active = activeEvents.find(id) != activeEvents.end() ? GetDarkmoonCoreString(LANG_ACTIVE) : "";
...
bool ChatHandler::HandleEventActiveListCommand(const char* /*args*/)
{
    ...
    char const* active = GetDarkmoonCoreString(LANG_ACTIVE);
...
bool ChatHandler::HandleEventInfoCommand(const char* args)
{
    ...
    char const* activeStr = active ? GetDarkmoonCoreString(LANG_ACTIVE) : "";
    ...
    for (uint32 id = 0; id < sCharTitlesStore.GetNumRows(); id++)
    {
        ...
        if (titleInfo)
        {
            ...
            if (!name.empty())
            {
                ...
                char const* knownStr = target && target->HasTitle(titleInfo) ? GetDarkmoonCoreString(LANG_KNOWN) : "";

                char const* activeStr = target && target->GetUInt32Value(PLAYER_CHOSEN_TITLE) == titleInfo->bit_index
                    ? GetDarkmoonCoreString(LANG_ACTIVE)
                    : "";
...
bool ChatHandler::HandleCharacterTitlesCommand(const char* args)
{
    ...
    char const* knownStr = GetDarkmoonCoreString(LANG_KNOWN);
