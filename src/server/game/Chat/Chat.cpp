...
const char *ChatHandler::GetDarkmoonCoreString(int32 entry) const
{
    return m_session->GetDarkmoonCoreString(entry);
}
...
{
    SendSysMessage(GetDarkmoonCoreString(entry));
}

void ChatHandler::PSendSysMessage(int32 entry, ...)
{
    const char *format = GetDarkmoonCoreString(entry);
    va_list ap;
    char str [2048];
    va_start(ap, entry);
...
const char *CliHandler::GetDarkmoonCoreString(int32 entry) const
{
...
{
    return GetDarkmoonCoreString(LANG_CONSOLE_COMMAND);
}
