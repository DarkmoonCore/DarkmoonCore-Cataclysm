...
bool ObjectMgr::LoadDarkmoonCoreStrings(char const* table, int32 min_value, int32 max_value)
{
    ...
    for (DarkmoonCoreStringLocaleMap::iterator itr = mDarkmoonCoreStringLocaleMap.begin(); itr != mDarkmoonCoreStringLocaleMap.end();)
    {
        if (itr->first >= start_value && itr->first < end_value)
            mDarkmoonCoreStringLocaleMap.erase(itr++);
        else
            ++itr;
    }
    ...
    {
        sLog->outString();
        if (min_value == MIN_DARKMOONCORE_STRING_ID)              // error only in case internal strings
            sLog->outErrorDb(">> Loaded 0 DarkmoonCore Strings. DB table `%s` is empty. Cannot continue.", table);
        else
    ...
    do
    {
        ...
        DarkmoonCoreStringLocale& data = mDarkmoonCoreStringLocaleMap[entry];
    ...
    sLog->outString();
    if (min_value == MIN_DARKMOONCORE_STRING_ID)
        sLog->outString(">> Loaded %u DarkmoonCore Strings from table %s", count, table);
    else
...
const char *ObjectMgr::GetDarkmoonCoreString(int32 entry, LocaleConstant locale_idx) const
{
    if (DarkmoonCoreStringLocale const *msl = GetDarkmoonCoreStringLocale(entry))
    {
    ...
    if (entry > 0)
        sLog->outErrorDb("Entry %i not found in `darkmooncore_string` table.", entry);
    else
    ...
    for (ScriptMapMap::const_iterator itrMM = scripts->begin(); itrMM != scripts->end(); ++itrMM)
    {
        for (ScriptMap::const_iterator itrM = itrMM->second.begin(); itrM != itrMM->second.end(); ++itrM)
        {
            switch(itrM->second.command)
            {
                case SCRIPT_COMMAND_TALK:
                {
                    if (!GetDarkmoonCoreStringLocale (itrM->second.Talk.TextID))
...
{
    LoadDarkmoonCoreStrings("db_script_string", MIN_DB_SCRIPT_STRING_ID, MAX_DB_SCRIPT_STRING_ID);

    std::set<int32> ids;

    for (int32 i = MIN_DB_SCRIPT_STRING_ID; i < MAX_DB_SCRIPT_STRING_ID; ++i)
        if (GetDarkmoonCoreStringLocale(i))
            ids.insert(i);
...
bool LoadDarkmoonCoreStrings(char const* table, int32 start_value, int32 end_value)
{
    ...
    return sObjectMgr->LoadDarkmoonCoreStrings(table, start_value, end_value);
}
