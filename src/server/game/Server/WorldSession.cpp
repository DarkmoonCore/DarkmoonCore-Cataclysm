...
{
    char const* format = GetDarkmoonCoreString(string_id);
    if (format)
    {
        va_list ap;
        char szStr [1024];
...
const char * WorldSession::GetDarkmoonCoreString(int32 entry) const
{
    return sObjectMgr->GetDarkmoonCoreString(entry, GetSessionDbLocaleIndex());
}
