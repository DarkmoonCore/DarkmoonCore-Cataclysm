            ...
            if (CanJoinConstantChannelInZone(channel, current_zone))
            {
                if (!(channel->flags & CHANNEL_DBC_FLAG_GLOBAL))
                {
                    ...
                    if (channel->flags & CHANNEL_DBC_FLAG_CITY_ONLY)
                        currentNameExt = sObjectMgr->GetDarkmoonCoreStringForDBCLocale(LANG_CHANNEL_CITY);
                    else
...
void Player::_LoadInventory(PreparedQueryResult result, uint32 timediff)
{
    ...
    if (result)
    {
        ...
        {
            std::string subject = GetSession()->GetDarkmoonCoreString(LANG_NOT_EQUIPPED_ITEM);
...
bool Player::Satisfy(AccessRequirement const* ar, uint32 target_map, bool report)
{
    if (!isGameMaster() && ar)
    {
        ...
        {
            GetSession()->SendAreaTriggerMessage("%s", GetSession()->GetDarkmoonCoreString(LANG_INSTANCE_CLOSED));
            return false;
        }
        ...
        if (LevelMin || LevelMax || missingItem || missingQuest || missingAchievement)
        {
            if (report)
            {
                if (missingQuest && !ar->questFailedText.empty())
                    ...
                    GetSession()->SendAreaTriggerMessage(GetSession()->GetDarkmoonCoreString(LANG_LEVEL_MINREQUIRED_AND_ITEM), LevelMin, ObjectMgr::GetItemPrototype(missingItem)->Name1);
                else if (LevelMin)
                    GetSession()->SendAreaTriggerMessage(GetSession()->GetDarkmoonCoreString(LANG_LEVEL_MINREQUIRED), LevelMin);
            }
            return false;
        }
    }
