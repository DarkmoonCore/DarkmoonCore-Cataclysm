...
void OPvPCapturePointZM_Beacon::ChangeState()
{
    ...
    if (m_OldState == OBJECTIVESTATE_ALLIANCE)
    {
        ...
        sWorld->SendZoneText(ZM_GRAVEYARD_ZONE, sObjectMgr->GetDarkmoonCoreStringForDBCLocale(ZMBeaconLooseA[m_TowerType]));
    }
    ...
    else if (m_OldState == OBJECTIVESTATE_HORDE)
    {
        ...
        sWorld->SendZoneText(ZM_GRAVEYARD_ZONE, sObjectMgr->GetDarkmoonCoreStringForDBCLocale(ZMBeaconLooseH[m_TowerType]));
    }
    ...
    switch(m_State)
    {
        case OBJECTIVESTATE_ALLIANCE:
            ...
            if (((OutdoorPvPZM*)m_PvP)->m_AllianceTowersControlled<ZM_NUM_BEACONS)
                ((OutdoorPvPZM*)m_PvP)->m_AllianceTowersControlled++;
            sWorld->SendZoneText(ZM_GRAVEYARD_ZONE, sObjectMgr->GetDarkmoonCoreStringForDBCLocale(ZMBeaconCaptureA[m_TowerType]));
            break;
        case OBJECTIVESTATE_HORDE:
            m_TowerState = ZM_TOWERSTATE_H;
            if (((OutdoorPvPZM*)m_PvP)->m_HordeTowersControlled<ZM_NUM_BEACONS)
                ((OutdoorPvPZM*)m_PvP)->m_HordeTowersControlled++;
            sWorld->SendZoneText(ZM_GRAVEYARD_ZONE, sObjectMgr->GetDarkmoonCoreStringForDBCLocale(ZMBeaconCaptureH[m_TowerType]));
            break;
    ...
    if (retval >= 0)
    {
        if (plr->HasAura(ZM_BATTLE_STANDARD_A) && m_GraveYardState != ZM_GRAVEYARD_A)
        {
            if (m_GraveYardState == ZM_GRAVEYARD_H)
                sWorld->SendZoneText(ZM_GRAVEYARD_ZONE, sObjectMgr->GetDarkmoonCoreStringForDBCLocale(LANG_OPVP_ZM_LOOSE_GY_H));
            ...
            plr->RemoveAurasDueToSpell(ZM_BATTLE_STANDARD_A);
            sWorld->SendZoneText(ZM_GRAVEYARD_ZONE, sObjectMgr->GetDarkmoonCoreStringForDBCLocale(LANG_OPVP_ZM_CAPTURE_GY_A));
        }
        else if (plr->HasAura(ZM_BATTLE_STANDARD_H) && m_GraveYardState != ZM_GRAVEYARD_H)
        {
            if (m_GraveYardState == ZM_GRAVEYARD_A)
                sWorld->SendZoneText(ZM_GRAVEYARD_ZONE, sObjectMgr->GetDarkmoonCoreStringForDBCLocale(LANG_OPVP_ZM_LOOSE_GY_A));
            ...
            plr->RemoveAurasDueToSpell(ZM_BATTLE_STANDARD_H);
            sWorld->SendZoneText(ZM_GRAVEYARD_ZONE, sObjectMgr->GetDarkmoonCoreStringForDBCLocale(LANG_OPVP_ZM_CAPTURE_GY_H));
        }
        UpdateTowerState();
    }
    return retval;
