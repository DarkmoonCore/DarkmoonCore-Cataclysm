    ...
    case SI_AREATRIGGER_A:
        if (plr->GetTeam() == ALLIANCE && plr->HasAura(SI_SILITHYST_FLAG))
        {
            ...
            if (m_Gathered_A >= SI_MAX_RESOURCES)
            {
                ...
                sWorld->SendZoneText(OutdoorPvPSIBuffZones[0], sObjectMgr->GetDarkmoonCoreStringForDBCLocale(LANG_OPVP_SI_CAPTURE_A));
    ...
    case SI_AREATRIGGER_H:
        if (plr->GetTeam() == HORDE && plr->HasAura(SI_SILITHYST_FLAG))
        {
            ...
            if (m_Gathered_H >= SI_MAX_RESOURCES)
            {
                ...
                sWorld->SendZoneText(OutdoorPvPSIBuffZones[0], sObjectMgr->GetDarkmoonCoreStringForDBCLocale(LANG_OPVP_SI_CAPTURE_H));
